...
def frac_diff_ffd(x, d, thres):
    output = []
    for i in range(1, len(x)):
        w = get_weight_ffd(d, thres, i)
        width = len(w) - 1
        x_window = x[i - width:i + 1]
        output.append(np.dot(w.T, x_window)[0])
...
def by_ticker(df, field="Share Price", type, lag, thresh):
